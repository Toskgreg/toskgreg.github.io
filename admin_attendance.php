<?php
   ...
   $fullname = "Enter Nrm member first and last name:";
   $button = "";
        ...
        if(preg_match('/\s/',$name) == 1){
                ...
                if(mysqli_num_rows($query)==1){
                    while($row = mysqli_fetch_assoc($query)){
                        $nrmid = $row["member_id"];
                        $fullname = "Nrm ID: <strong>".$nrmid."</strong> Name: <strong>".$row["member_fname"]." ".$row["member_lname"]."</strong>";

                        $sql2 = "SELECT member_id, attendance_time FROM attendance WHERE member_id = {$nrmid} AND attendance_date = '".date("Y-m-d")."'";
                        $query2 = mysqli_query($conn,$sql2);

                        if(mysqli_num_rows($query2)==0){
                        $button = '<form method = "POST" action = "admin_attendance.php"><input type="hidden" value="'.$nrmid.'" name="nrmid" required> <button class="w3-button w3-grey w3-section w3-padding" type="submit" name="check"><span class="fa fa-check-square"></span><strong> Check Attendance</strong></button></form> ';
                        }else{
   ...
   if(isset($_POST['check'])){
       $nrmid = mysqli_real_escape_string($conn, $_POST['nrmid']);
       $sql = "INSERT INTO attendance(member_id, attendance_date, attendance_time) VALUES ('{$nrmid}','".date("Y-m-d")."','".date("g:i:s")."')";
       $query = mysqli_query($conn,$sql);
   }
   ...
   ?>
    ...
    <head>
        <title>Admin Attendance | NIM</title>
